...
def main():
    ...
    for i_sample in id_std:
        ...
        if gt_data[i_sample]["num"] != 0 and pred_data[i_sample]["num"] != 0:
            ...
            pre = tp_pred_index.shape[0] / (
                tp_pred_index.shape[0] + fp_pred_index.shape[0] + 1e-20
            )
            rec = tp_pred_index.shape[0] / (
                tp_pred_index.shape[0] + fn_gt_index.shape[0] + 1e-20
            )
        ...
        print(
            len(cor_index[0]), len(cor_index[1]), max(cor_index[0]), max(cor_index[1])
        )
        ...
        for l in range(len(cor_index[0])):
            ...
            cv2.line(
                img, (pre_cor[0], pre_cor[1]), (gt_cor[0], gt_cor[1]), (255, 0, 0), 1
            )
