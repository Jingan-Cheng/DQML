...
class MultiScaleQuantumBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_scales=4, qm_channels=16, n_wires=4):
        super().__init__()
        ...
        self.conv_scales = nn.ModuleList([
            DepthwiseSeparableConv(in_channels, in_channels // 4, kernel_size=3, padding=rate, dilation=rate)
            for rate in dilation_rates
        ])
        self.quantum_layer =  QFCModel(in_channel=in_channels, qm_channel=qm_channels)
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.conv_scales):
            ...
            multi_scale_outputs.append(scale_features)
...
class QFCModel(tq.QuantumModule):
    class QLayer(tq.QuantumModule):
        def __init__(self):
            ...
            self.fixed_op_list = [
                ...
                tq.CRY(has_params=True, trainable=True)
            ]
            ...
            self.op_wires = [
                ...
                [1],  # RY on wire 1
                [2],  # RZ on wire 2
                [0, 2],  # CRX between wire 0 and wire 2
                [1, 3]   # CRY between wire 1 and wire 3
            ]
    ...
    def __init__(self, in_channel, qm_channel=16):
        super().__init__()
        self.n_wires = 4
